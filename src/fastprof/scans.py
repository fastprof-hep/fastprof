...
from .plot_tools import PlotBands
...
class Scan1D (Scan) :
  ...
  def spline(self, order : int = 3) :
    ...
    """
    pts = self.points()
    return scipy.interpolate.InterpolatedUnivariateSpline(pts[0], pts[1][0], k=order)
...
class UpperLimitScan (Scan1D):
  ...
  def plot(self, canvas : tuple = (None, None), marker : str = 'b', label : str = None, with_errors : bool = False, bands : int = None) :
    ...
    Args:
      ...
      bands : if not None, plot -bands...+bands error bands
    """
    if canvas == (None, None) : 
